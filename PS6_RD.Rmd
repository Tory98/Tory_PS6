...
cuts<-c(seq(-.02,0-.02/30, by=(.02/30)), seq(0+(.02/30),.02, by=(.02/15)))
#cuts

midpoints<-cuts[2:45]-(.02/60)
#midpoints
...
fig2_data<-fig2_data%>%dplyr::group_by(bins)%>%dplyr::summarise(mean_inc=mean(pred_inc), mean_rec_panes=mean(received), mean_gov_sup07=mean(scaled_y_07), mean_gov_sup08=mean(scaled_y_08))
